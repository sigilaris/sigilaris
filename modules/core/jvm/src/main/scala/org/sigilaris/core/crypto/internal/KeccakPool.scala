...
/** Thread-local pool of Keccak-256 digest instances for efficient reuse.
  *
  * Provides thread-safe access to Keccak-256 digest instances without requiring
  * allocation on every hash operation. Each thread maintains its own instance,
  * and the digest is reset before each use.
  *
  * @note
...
object KeccakPool:
  ...
  private val tl: ThreadLocal[Keccak.Digest256] =
    new ThreadLocal[Keccak.Digest256]():
