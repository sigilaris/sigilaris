...
import _root_.org.sigilaris.core.application.{state as appState}
import _root_.org.sigilaris.core.application.support.{compiletime as supportCT, encoding as supportEnc}
...
class PrefixCollisionTest extends FunSuite:
  ...
  test("PrefixFreePath should reject identical table names in same schema"):
    ...
    val errors = compileErrors("""
      import _root_.org.sigilaris.core.application.{state as appState}
      import _root_.org.sigilaris.core.application.support.compiletime as supportCT
      import _root_.org.sigilaris.core.datatype.Utf8

      type DuplicateSchema = appState.Entry["accounts", Utf8, Long] *:
             appState.Entry["accounts", Utf8, Utf8] *:
                             EmptyTuple
      summon[supportCT.PrefixFreePath["app" *: EmptyTuple, DuplicateSchema]]
    """)
  ...
  test("PrefixFreePath should reject table names with prefix relationship"):
    ...
    type PrefixRelatedSchema = appState.Entry["user", Utf8, Long] *:
            appState.Entry["user_data", Utf8, Utf8] *:
                                EmptyTuple

    // This test verifies our encoding is actually prefix-free
    // With length-prefix encoding, these should be OK
    val evidence = summon[supportCT.PrefixFreePath["app" *: EmptyTuple, PrefixRelatedSchema]]
  ...
  test("PrefixFreePath should handle same table name at different path depths"):
    type Schema1 = appState.Entry["data", Utf8, Long] *: EmptyTuple
    type Schema2 = appState.Entry["data", Utf8, Utf8] *: EmptyTuple

    // Same table name but different paths should be OK
    val evidence1 = summon[supportCT.PrefixFreePath[("app", "v1"), Schema1]]
    val evidence2 = summon[supportCT.PrefixFreePath[("app", "v2"), Schema2]]
  ...
  test("PrefixFreePath should reject composition with duplicate table names"):
    val errors = compileErrors("""
      ...
      type Schema1 = appState.Entry["balances", Utf8, Long] *: EmptyTuple
      type Schema2 = appState.Entry["balances", Utf8, Utf8] *: EmptyTuple
      type Combined = Schema1 ++ Schema2
      summon[supportCT.PrefixFreePath["app" *: EmptyTuple, Combined]]
    """)
  ...
  test("PrefixFreePath should accept valid prefix-free schema"):
    type ValidSchema = appState.Entry["accounts", Utf8, Long] *:
           appState.Entry["balances", Utf8, Long] *:
           appState.Entry["metadata", Utf8, Utf8] *:
                       EmptyTuple

    val evidence = summon[supportCT.PrefixFreePath["app" *: EmptyTuple, ValidSchema]]
  ...
  test("PrefixFreePath should accept empty schema"):
    val evidence = summon[supportCT.PrefixFreePath["app" *: EmptyTuple, EmptyTuple]]
  ...
  test("PrefixFreePath should accept single-table schema"):
    type SingleSchema = appState.Entry["accounts", Utf8, Long] *: EmptyTuple
    val evidence = summon[supportCT.PrefixFreePath["app" *: EmptyTuple, SingleSchema]]
  ...
  test("Runtime validator: detect byte-level prefix collisions"):
    ...
    val prefix1 = supportEnc.tablePrefix[("app" *: EmptyTuple), "accounts"]
    val prefix2 = supportEnc.tablePrefix[("app" *: EmptyTuple), "balances"]
    val prefix3 = supportEnc.tablePrefix[("app" *: EmptyTuple), "metadata"]
  ...
  test("composeBlueprint should validate prefix-free property"):
    ...
    val errors = compileErrors("""
      import cats.Id
      import _root_.org.sigilaris.core.application.{state as appState}
      import _root_.org.sigilaris.core.application.state.{StoreF, Tables}
      ...
      import _root_.org.sigilaris.core.application.transactions.model.{Signed, Tx, TxRegistry}
      import _root_.org.sigilaris.core.datatype.Utf8

      type Schema1 = appState.Entry["data", Utf8, Long] *: EmptyTuple
      type Schema2 = appState.Entry["data", Utf8, Utf8] *: EmptyTuple

      val bp1Entry = appState.Entry["data", Utf8, Long]
      ...
      val bp2Entry = appState.Entry["data", Utf8, Utf8]
  ...
  test("PrefixFreeValidator.validateSchema compile-time check"):
    ...
    type Schema = appState.Entry["balances", Utf8, Long] *: appState.Entry["accounts", Utf8, Utf8] *: EmptyTuple
