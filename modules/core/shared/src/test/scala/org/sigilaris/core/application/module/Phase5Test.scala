...
/** Tests for Phase 5: Assembly (extend, mergeReducers, ModuleFactory).
  ...
  *   - ModuleFactory: safe for self-contained modules (Deps = EmptyTuple
  *     enforced)
  ...
  *   - extend combines schemas, transactions, dependencies, and reducers
  *     correctly
  *   - Factory pattern: build → build → extend (for self-contained modules)
  *
  * Note: Uses SyncIO for cross-platform compatibility (JVM + JS). Note:
  * aggregate was removed (blocked on subset derivation, see ADR-0009).
  */
class Phase5Test extends FunSuite:
  ...
  type GroupSchema = Entry["groups", Address, GroupInfo] *:
    Entry["members", Address, BigNat] *: EmptyTuple
  ...
  def createAccountsBlueprint(): ModuleBlueprint[
    SyncIO,
    "accounts",
    AccountsSchema,
    EmptyTuple,
    ...
    val reducer = new StateReducer0[SyncIO, AccountsSchema, EmptyTuple]:
      def apply[T <: Tx](signedTx: Signed[T])(using
        ...
        StateT.pure:
          (
            ().asInstanceOf[signedTx.value.Result],
  ...
  def createGroupBlueprint()
      : ModuleBlueprint[SyncIO, "group", GroupSchema, EmptyTuple, EmptyTuple] =
  ...
  test(
    "Shared assembly: single Accounts mounted, both modules use same tables",
  ):
    given merkle.MerkleTrie.NodeStore[SyncIO] = createNodeStore()
    given cats.Monad[SyncIO]                  = cats.effect.Sync[SyncIO]
  ...
  test(
    "Reducer merging: r1 succeeds - transaction executed and result returned",
  ):
    given merkle.MerkleTrie.NodeStore[SyncIO] = createNodeStore()
    given cats.Monad[SyncIO]                  = cats.effect.Sync[SyncIO]
        ...
        signedTx.value match
          case CreateAccount(addr, acc) =>
            ...
            StateT.pure:
              (
                ().asInstanceOf[signedTx.value.Result],
                List(AccountCreated(addr).asInstanceOf[signedTx.value.Event]),
              )
          case _ =>
            import cats.data.StateT
            StateT.pure:
    ...
    val tx =
      CreateAccount(Address(Utf8("addr1")), AccountData(Utf8("account1")))
    val signedTx     = signTx(tx, account)
    val initialState = StoreState.empty

    val result =
      extended.reducer.apply(signedTx).run(initialState).value.unsafeRunSync()
          ...
          case _: CreateGroup =>
            ...
            StateT.liftF:
              EitherT.leftT[
                ...
                _root_.org.sigilaris.core.failure
                  .TrieFailure("Accounts module cannot handle CreateGroup"),
              )
          case _ =>
            import cats.data.StateT
            StateT.pure:
        ...
        signedTx.value match
          case CreateGroup(addr, grp) =>
            ...
            StateT.pure:
              (
                ().asInstanceOf[signedTx.value.Result],
                List(GroupCreated(addr).asInstanceOf[signedTx.value.Event]),
              )
          case _ =>
            import cats.data.StateT
            StateT.pure:
  ...
  test("Reducer merging: r1 succeeds with empty events - no fallback to r2"):
    ...
    val account = Account.Named(Utf8("testuser"))
    val tx =
      CreateAccount(Address(Utf8("addr3")), AccountData(Utf8("account3")))
    val signedTx     = signTx(tx, account)
    val initialState = StoreState.empty
    ...
    result match
      case Right((finalState, (txResult, txEvents))) =>
        ...
        assertEquals(
          r2Called,
          false,
          "r2 should not be called when r1 succeeds with empty events",
        )
      case Left(err) =>
