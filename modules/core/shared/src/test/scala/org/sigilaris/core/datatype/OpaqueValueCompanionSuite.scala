...
import org.sigilaris.core.codec.byte.{ByteCodec, ByteDecoder, ByteEncoder, DecodeResult}
...
final class OpaqueValueCompanionSuite extends FunSuite:
  ...
  given wrappedPairReprByteCodec: ByteCodec[WrappedPairRepr] with
    ...
    def decode(
        bytes: ByteVector,
    ): Either[org.sigilaris.core.failure.DecodeFailure, DecodeResult[WrappedPairRepr]] =
  ...
  object WrappedPair extends OpaqueValueCompanion[WrappedPair, WrappedPairRepr]:
    def apply(name: Utf8, nonce: BigNat): WrappedPair = WrappedPairRepr(name, nonce)
  ...
  test("GroupId forwards byte/json codecs and JsonKeyCodec from Utf8"):
    ...
    assertEquals(ByteDecoder[GroupId].decode(bytes).map(_.value), Right(groupId))
    assertEquals(
      decodedWithRemainder,
      Right(DecodeResult(groupId, ByteVector(0x01.toByte))),
    )
    assertEquals(json, JsonValue.obj("developers" -> JsonValue.JNumber(BigDecimal(1))))
    assertEquals(JsonDecoder[Map[GroupId, Int]].decode(json), Right(Map(groupId -> 1)))
  ...
  test("NetworkId forwards byte and json codecs from BigNat"):
    ...
    assertEquals(ByteDecoder[NetworkId].decode(bytes).map(_.value), Right(networkId))
  ...
  test("ByteCodec.opaqueProduct derives a codec for opaque wrappers over product repr"):
    val value = WrappedPair(Utf8("box"), BigNat.One)
    val bytes = WrappedPair.byteCodec.encode(value)

    assertEquals(
      WrappedPair.byteCodec.decode(bytes),
      Right(org.sigilaris.core.codec.byte.DecodeResult(value, scodec.bits.ByteVector.empty)),
    )
