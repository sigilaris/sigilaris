...
final class JsonMapSuite extends FunSuite:
  ...
  test("encode Map[Int, String] via JsonKeyCodec[Int]"):
    ...
    assertEquals(
      json,
      JsonValue.obj(
        "1" -> JsonValue.JString("a"),
        "2" -> JsonValue.JString("b"),
      ),
    )

  test("decode Map[Int, String] and fail on bad key"):
    val json = JsonValue.obj("x" -> JsonValue.JString("a"))
    val res  = JsonDecoder[Map[Int, String]].decode(json)
    assert(res.isLeft)

  test("decode Map[Int, String] and fail on duplicate normalized keys"):
    val json = JsonValue.obj(
      "1"  -> JsonValue.JString("a"),
      "01" -> JsonValue.JString("b"),
    )
    val res = JsonDecoder[Map[Int, String]].decode(json)
    assert(res.isLeft)

  test(
    "encode/decode Map[String, Int] remains supported via JsonKeyCodec[String]",
  ):
    val m    = Map("a" -> 1, "b" -> 2)
    val json = m.toJson
    assertEquals(
      json,
      JsonValue.obj(
        "a" -> JsonValue.JNumber(BigDecimal(1)),
        "b" -> JsonValue.JNumber(BigDecimal(2)),
      ),
    )
    val round = JsonDecoder[Map[String, Int]].decode(json)
  ...
  test("encode/decode Map[UUID, Int]"):
    ...
    val decoded = JsonDecoder[Map[UUID, Int]].decode(json)
