...
/** High-level DSL for mounting blueprints at specific paths.
  *
  * Provides convenient methods for mounting module blueprints and composed
  * blueprints at paths, with automatic evidence requirements:
  *
  * Single-name mounting:
  *   - [[mount]] - mount a
  *     [[org.sigilaris.core.application.module.blueprint.ModuleBlueprint]]
  *   - [[mountComposed]] - mount a
  ...
  *
  * val module = mount("accounts" -> accountsBlueprint) val composed =
  * mountComposed("app" -> composedBlueprint) }}}
  *
  * @see
...
object BlueprintDsl:
  ...
  /** Mount a module blueprint at a single-segment path.
    ...
    *   the effect type
    * @tparam Name
    *   the mount path (singleton string literal)
    * @tparam MName
    *   the module name
    * @tparam Owns
    *   the owned schema tuple
    * @tparam Needs
    *   the needed schema tuple
    * @tparam Txs
    *   the transaction types tuple
    * @param binding
    *   tuple of (path name, blueprint)
    * @return
  ...
  /** Mount a module blueprint at a multi-segment path.
    ...
    *   the effect type
    * @tparam Path
    *   the mount path tuple (e.g., ("app", "v1"))
    * @tparam MName
    *   the module name
    * @tparam Owns
    *   the owned schema tuple
  ...
  /** Mount a composed blueprint at a single-segment path.
    ...
    *   the transaction types tuple
    * @param binding
    *   tuple of (path name, composed blueprint)
    * @return
  ...
  /** Mount a composed blueprint at a multi-segment path.
    ...
    *   the transaction types tuple
    * @param binding
    *   tuple of (path tuple, composed blueprint)
    * @return
    *   a mounted StateModule with RoutedStateReducer
    */
