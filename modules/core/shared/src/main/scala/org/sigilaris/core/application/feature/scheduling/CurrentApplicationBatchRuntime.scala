...
import org.sigilaris.core.application.execution.{StateModuleExecutor, TxExecution}
...
import org.sigilaris.core.application.scheduling.{BatchPlan, BatchPlanner, ClassifiedItem, CompatibilityBatchPlan, CompatibilityMode, CompatibilityReason, FootprintConflict, SchedulableBatchExecutor, SchedulableBatchPlan, SchedulableExecutionFailure, SchedulingClassification}
...
enum CurrentApplicationBatchRejected:
  ...
  case SchedulableExecutionFailed(
      failure: SchedulableExecutionFailure[CurrentApplicationSignedTx, SigilarisFailure],
      diagnostics: CurrentApplicationBatchDiagnostics,
    )
...
final class CurrentApplicationBatchRuntime private (
  ...
  def applyBatch(
    ...
    state.receiptsByIdempotencyKey.get(batch.idempotencyKey) match
      ...
      case None =>
        ...
        BatchPlanner.planClassified(classified).leftMap:
          conflict =>
            CurrentApplicationBatchRejected.SchedulingConflict(
              conflict = conflict,
              diagnostics = schedulableDiagnostics(classified, duplicatesDropped),
            )
        .flatMap:
      ...
      .map: execution =>
        ...
        val nextState =
          state.copy(
            ...
            receiptsByIdempotencyKey =
              state.receiptsByIdempotencyKey.updated(batch.idempotencyKey, receipt),
          )
    ...
    plan.items
      .foldLeft[
        ...
        case (Right((currentState, executed)), classifiedItem) =>
          executeTx(currentState, classifiedItem.item).leftMap:
            cause =>
          ...
          .map: execution =>
            execution.nextState ->
              (executed :+ CurrentApplicationExecutedTx(classifiedItem.item, execution))
        case (left @ Left(_), _) =>
          left
  ...
  private def compatibilityDiagnostics(
    ...
    CurrentApplicationBatchDiagnostics(
      mode =
        CurrentApplicationBatchMode.Compatibility(
  ...
  private def compatibilityReason(
    ...
    val compatibilityReasons =
      plan.items.collect:
        case ClassifiedItem(_, SchedulingClassification.Compatibility(reason)) =>
          reason
      ...
      case CompatibilityMode.CompatibilityOnly =>
        compatibilityReasons.headOption match
          case Some(head) =>
            head.copy(
              detail =
                (head.detail.toList ++ compatibilityReasons.drop(1).map(renderCompatibilityReason))
                  .distinct
                  .mkString(",")
                  .some
                  .filter(_.nonEmpty),
    ...
    items.foldLeft(
      ...
      case ((unique, seen, duplicates), item) =>
        if seen.contains(item) then
          (unique, seen, duplicates :+ item)
        else
          (unique :+ item, seen + item, duplicates)
    match
