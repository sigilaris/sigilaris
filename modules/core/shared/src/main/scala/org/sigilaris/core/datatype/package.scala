...
/** Core data types with specialized encoding and type safety.
  *
  * This package provides opaque types for common data structures used
  * throughout Sigilaris, each with carefully designed byte and JSON codecs. All
  * types are zero-cost abstractions that provide compile-time safety without
  * runtime overhead.
  *
  * =Available Types=
  *
  ...
  *
  * '''JSON Encoding:'''
  *   - BigNat: String by default (configurable via
  *     [[org.sigilaris.core.codec.json.JsonConfig]])
  *   - UInt256: Lowercase hex string without `0x` prefix
  *   - Utf8: Plain JSON string
  *
  * @see
  *   [[org.sigilaris.core.codec.byte]] for byte codec details
  * @see
  *   [[org.sigilaris.core.codec.json]] for JSON codec details
  * @see
  *   [[org.sigilaris.core.failure]] for typed failure cases
  */
