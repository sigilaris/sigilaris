...
/** OrderedCodec extends ByteCodec with an ordering preservation law.
  ...
  * blockchain applications where prefix-free encoding and key ordering matter.
  *
  * The law states: For all x, y of type A, sign(compare(x, y)) ==
  * sign(encode(x).compare(encode(y)))
  *
  * Where sign(n) is:
  *   - negative if n < 0
  *   - zero if n == 0
...
trait OrderedCodec[A] extends ByteCodec[A], Ordering[A]:
  /** Verifies the ordering preservation law for two values.
    ...
    *   second value
    * @return
    *   true if the law holds for this pair
    */
...
object OrderedCodec:
  ...
  /** Creates an OrderedCodec from existing ByteCodec and Ordering instances.
    ...
    *
    * @param codec
    *   the ByteCodec instance
    * @param ord
    *   the Ordering instance
    * @return
    *   OrderedCodec that delegates to the given instances
    */
  def fromCodecAndOrdering[A](
      codec: ByteCodec[A],
      ord: Ordering[A],
  ): OrderedCodec[A] =
    new OrderedCodec[A]:
      def encode(a: A): ByteVector = codec.encode(a)
      def decode(bv: ByteVector): Either[DecodeFailure, DecodeResult[A]] =
        codec.decode(bv)
  ...
  given orderedByteVector: OrderedCodec[ByteVector] =
    new OrderedCodec[ByteVector]:
      ...
      def decode(
          bv: ByteVector,
      ): Either[DecodeFailure, DecodeResult[ByteVector]] =
        Right[DecodeFailure, DecodeResult[ByteVector]](
          DecodeResult(bv, ByteVector.empty),
        )
