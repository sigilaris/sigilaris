...
/** Accumulated reads and writes seen so far during batch conflict checking.
  *
  * @param readsSeen
  *   all state references read by previously accepted items
  * @param writesSeen
  *   all state references written by previously accepted items
  */
final case class AggregateFootprint(
    readsSeen: Set[StateRef],
    writesSeen: Set[StateRef],
):
  /** Attempts to accept a new item's footprint into the aggregate, failing on
    * conflict.
    *
    * @tparam A
    *   the item type
    * @param item
    *   the item being accepted (used in conflict reporting)
    * @param footprint
    *   the item's conflict footprint
    * @return
    *   either a conflict or the updated aggregate
    */
  def accept[A](
      item: A,
...
/** Describes a detected conflict between a new item's footprint and the
  * aggregate.
  *
  * @tparam A
  *   the item type
  * @param item
  *   the later-processed item that failed admission
  * @param stateRef
  *   the state reference where the conflict occurred
  * @param kind
  *   the kind of conflict (write-write or read-write)
  */
...
object ConflictFootprintVerifier:
  /** Verifies all items are conflict-free by sequentially accepting each into
    * an aggregate.
    *
    * @tparam A
    *   the item type
    * @param items
    *   the items with their footprints to verify
    * @return
    *   either the first conflict found or the final aggregate footprint
    */
  def verifyAll[A](
...
/** Failure indicating the actual footprint exceeded the declared footprint.
  *
  * @param unexpectedReads
  *   state references read that were not declared
  * @param unexpectedWrites
  *   state references written that were not declared
  */
final case class FootprintConformanceFailure(
  ...
  /** Returns true if there are no unexpected reads or writes.
    *
    * @return
    *   true if this failure is empty (no violations)
    */
  def isEmpty: Boolean =
    unexpectedReads.isEmpty && unexpectedWrites.isEmpty

/** Validates that an actual footprint conforms to (is a subset of) a declared
  * footprint.
  */
object ConflictFootprintConformance:
  /** Validates actual reads/writes are subsets of declared reads/writes.
    *
    * @param actual
    *   the actual footprint observed during execution
    * @param declared
    *   the declared footprint from the scheduling phase
    * @return
