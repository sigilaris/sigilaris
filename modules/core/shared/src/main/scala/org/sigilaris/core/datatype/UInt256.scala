...
/** 256-bit unsigned integer with fixed-size representation.
  *
  * UInt256 is an opaque type wrapping a 32-byte [[scodec.bits.ByteVector]]
  * stored in big-endian format. It represents unsigned integers in the range
  * [0, 2^256 - 1] with a fixed memory footprint.
  *
  * Key features:
  ...
  *   // From hex string
  *   val u1 =
  *     UInt256.fromHex("ff") // Right(UInt256 with 32 bytes, left-padded)
  ...
  *
  * @note
  *   Byte encoding is fixed-size (32 bytes). Values are stored in big-endian
  *   format.
  * @note
  *   JSON encoding uses lowercase hex strings without `0x` prefix for
  *   consistency.
  * @note
  *   The hex decoder accepts optional `0x` prefix, whitespace, and underscores
  *   for flexibility.
  *
  * @see
  *   [[org.sigilaris.core.failure.UInt256Failure]] for typed failure cases
  * @see
  *   [[org.sigilaris.core.codec.byte.ByteEncoder]] for byte encoding details
  * @see
...
object UInt256:
  ...
  /** Creates a UInt256 from big-endian bytes, left-padding with zeros if
    * shorter.
    *
    * @param bytes
    *   the input bytes in big-endian format
    * @return
    *   `Right(UInt256)` if bytes.length ≤ 32, `Left(UInt256TooLong)` if
    *   bytes.length > 32
    *
    * @example
    *   ```scala
    ...
    *
    * @note
    *   Input shorter than 32 bytes is left-padded with zeros.
    * @see
    *   [[unsafeFromBytesBE]] for throwing version
    */
  ...
  /** Unsafe constructor from big-endian bytes, throwing on invalid input.
    ...
    *   the input bytes in big-endian format
    * @return
    *   UInt256 if bytes.length ≤ 32
    * @throws IllegalArgumentException
    *   if bytes.length > 32
    *
    * @note
    *   Use [[fromBytesBE]] for safe construction. This is intended for
    *   constants and test data.
    * @see
    *   [[fromBytesBE]] for safe version
    */
  ...
  /** Constructs a UInt256 from an unsigned BigInt.
    *
    * @param n
    *   the unsigned BigInt value (must satisfy 0 ≤ n < 2^256)
    * @return
    *   `Right(UInt256)` if valid, `Left(UInt256Failure)` otherwise
    *
    * @example
    *   ```scala
    ...
    *   UInt256.fromBigIntUnsigned(BigInt(-1)) // Left(UInt256NegativeValue)
    *   UInt256.fromBigIntUnsigned(
    *     BigInt(2).pow(256),
    *   ) // Left(UInt256Overflow(...))
    *   ```
    *
    * @note
    *   Fails with [[org.sigilaris.core.failure.UInt256NegativeValue]] if n < 0
    * @note
    *   Fails with [[org.sigilaris.core.failure.UInt256Overflow]] if n ≥ 2^256
    * @see
    *   [[unsafeFromBigIntUnsigned]] for throwing version
    */
  ...
  /** Constructs a UInt256 from an unsigned java.math.BigInteger (JVM
    * fast-path).
    *
    * This is a JVM-optimized fast-path for interoperability with
    * BigInteger-based cryptographic operations. Validates that the input is
    * non-negative and fits within 256 bits.
    *
    * @param value
    *   the unsigned BigInteger value (must satisfy 0 ≤ value < 2^256)
    * @return
    *   `Right(UInt256)` if valid, `Left(UInt256Failure)` otherwise
    *
    * @example
    ...
    *   ) // Left(UInt256NegativeValue)
    *   UInt256.fromBigIntegerUnsigned(
    *     BigInteger.TWO.pow(256),
    *   ) // Left(UInt256Overflow(...))
    *   ```
    *
    * @note
    *   Fails with [[org.sigilaris.core.failure.UInt256NegativeValue]] if
    *   value.signum < 0
    * @note
    *   Fails with [[org.sigilaris.core.failure.UInt256Overflow]] if
    *   value.bitLength > 256
    */
  def fromBigIntegerUnsigned(
  ...
  /** Creates a UInt256 from a hex string with flexible parsing.
    *
    * Accepts hex strings with optional `0x` prefix, whitespace, and
    * underscores. Empty string is treated as zero.
    *
    * @param hex
    ...
    *
    * @note
    *   Delegates to `ByteVector.fromHexDescriptive` for parsing
    * @note
    *   Result is left-padded to 32 bytes
    * @note
    *   Fails with [[org.sigilaris.core.failure.UInt256InvalidHex]] on invalid
    *   hex characters
    * @note
    *   Fails with [[org.sigilaris.core.failure.UInt256TooLong]] if decoded
    *   bytes exceed 32
    */
  def fromHex(hex: String): Either[UInt256Failure, UInt256] =
    for
      parsed <- ByteVector
        .fromHexDescriptive(hex.trim)
        .leftMap(msg => UInt256InvalidHex(msg))
      value <- Either.cond(
        parsed.length <= Size,
        leftPadTo32(parsed),
  ...
  /** Unsafe constructor from unsigned BigInt, throwing on invalid input.
    *
    * @param n
    *   the unsigned BigInt value
    * @return
    *   UInt256 if 0 ≤ n < 2^256
    * @throws IllegalArgumentException
    *   if n < 0 or n ≥ 2^256
    *
    * @note
    *   Use [[fromBigIntUnsigned]] for safe construction. This is intended for
    *   constants and test data.
    * @see
    *   [[fromBigIntUnsigned]] for safe version
    */
  @SuppressWarnings(Array("org.wartremover.warts.Throw"))
  ...
  /** Returns the underlying 32-byte big-endian representation.
    ...
    *   the UInt256 value
    * @return
    *   32-byte ByteVector in big-endian format
    */
  extension (u: UInt256)
    ...
    /** Converts to unsigned java.math.BigInteger (JVM fast-path).
      *
      * This is a JVM-optimized fast-path for interoperability with
      * BigInteger-based cryptographic operations. Converts the 32-byte
      * big-endian representation to a non-negative BigInteger.
      *
      * @return
      *   BigInteger in range [0, 2^256 - 1]
      *
      * @example
      *   ```scala
  ...
  /** Byte encoder for UInt256.
    ...
    * @see
    *   [[uint256ByteDecoder]] for decoding
    */
  ...
  /** Byte decoder for UInt256.
    ...
    *   decoder instance
    * @note
    *   Fails with [[org.sigilaris.core.failure.DecodeFailure]] if fewer than 32
    *   bytes available
    * @see
    *   [[uint256ByteEncoder]] for encoding
    */
  ...
  /** JSON encoder for UInt256.
    ...
    *
    * @note
    *   Always produces a 64-character hex string (32 bytes * 2 hex digits)
    * @see
    *   [[uint256JsonDecoder]] for decoding
    */
  ...
  /** JSON decoder for UInt256.
    *
    * Decodes from hex string, accepting optional `0x` prefix, whitespace, and
    * underscores.
    *
    * @return
    *   decoder instance
    ...
    *
    * @note
    *   Fails with [[org.sigilaris.core.failure.DecodeFailure]] on invalid hex
    *   or overflow
    * @see
    *   [[uint256JsonEncoder]] for encoding
    */
  ...
  /** JSON key codec for UInt256.
    ...
    * Decodes from hex string keys, accepting optional `0x` prefix, whitespace,
    * and underscores.
    *
    * @return
    *   key codec instance
    ...
    *
    * @note
    *   Keys are encoded as 64-character hex strings (32 bytes * 2 hex digits)
    * @note
    *   Fails with [[org.sigilaris.core.failure.DecodeFailure]] on invalid hex
    *   or overflow
    * @see
    *   [[uint256JsonEncoder]] for value encoding
    * @see
    *   [[uint256JsonDecoder]] for value decoding
    */
  ...
  /** OrderedCodec instance for UInt256.
    *
    * UInt256's fixed 32-byte big-endian encoding preserves ordering: smaller
    * unsigned integers encode to lexicographically smaller byte sequences.
    *
    * The big-endian byte representation satisfies the ordering law: compare(x,
    * y) ≡ encode(x).compare(encode(y))
    *
    * @return
    *   OrderedCodec instance
    * @see
    *   [[org.sigilaris.core.codec.OrderedCodec]] for law details
    */
  given uint256OrderedCodec: org.sigilaris.core.codec.OrderedCodec[UInt256] =
    new org.sigilaris.core.codec.OrderedCodec[UInt256]:
      ...
      def decode(bv: ByteVector): Either[
        DecodeFailure,
        org.sigilaris.core.codec.byte.DecodeResult[UInt256],
      ] =
