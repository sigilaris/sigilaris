...
/** Typeclass for instantiating tables from Entry instances.
  *
  * This uses the typeclass derivation pattern to avoid inline match issues
  * with abstract type parameters. Given instances are automatically derived
  * for EmptyTuple and cons cases at compile time.
  *
  * @tparam F the effect type
  * @tparam Path the mount path tuple
  * @tparam Schema the schema tuple type
  */
trait SchemaMapper[F[_], Path <: Tuple, Schema <: Tuple]:
  /** Instantiate tables from a schema tuple of Entry instances.
    *
    * @param schema the runtime tuple of Entry instances
    * @return a tuple of StateTable instances
    */
...
object SchemaMapper:
  ...
  /** Inductive case: process head Entry and recurse on tail.
    ...
    * 1. Compute the table prefix from Path and table name N
    * 2. Create a StateTable using Entry.createTable
    * 3. Recursively process the tail
    * 4. Cons the results together
    */
  given cons[F[_]: Monad, Path <: Tuple, N <: String, K, V, Tail <: Tuple](using
    ...
    def instantiate(schema: Entry[N, K, V] *: Tail): Tables[F, Entry[N, K, V] *: Tail] =
...
object SchemaInstantiation:
  /** Instantiate tables from a schema tuple of Entry instances.
    ...
    *
    * Note: The Monad[F] and NodeStore[F] constraints are not explicitly listed here
    * because they are required by the SchemaMapper instance itself.
    *
    * @tparam F the effect type
    * @tparam Path the mount path tuple
    * @tparam Schema the schema tuple type
    * @param schema the runtime tuple of Entry instances
    * @param mapper the SchemaMapper instance (automatically derived)
    * @return a tuple of StateTable instances
    */
