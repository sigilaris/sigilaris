...
/** Non-negative arbitrary-precision integer (Natural number).
  *
  * BigNat is an opaque type wrapping a refined BigInt constrained to
  * non-negative values (≥ 0). It provides arithmetic operations that preserve
  * the non-negative invariant and safely handles operations that could violate
  * this constraint.
  *
  * Key features:
  *   - Zero-cost abstraction over BigInt with compile-time safety
  *   - Arithmetic operations (addition, multiplication, division) always return
  *     valid BigNat
  *   - Subtraction returns `Either[String, BigNat]` to handle potential
  *     negative results
  *   - JSON codec encodes/decodes as string by default (configurable via
  *     [[org.sigilaris.core.codec.json.JsonConfig]])
  ...
  *
  * @note
  *   This type uses variable-length byte encoding. The encoded size depends on
  *   the magnitude of the value.
  * @note
  *   JSON codec behavior is controlled by
  *   [[org.sigilaris.core.codec.json.JsonConfig]]. By default, values are
  *   encoded as JSON strings and decoders accept both string and numeric
  *   representations.
  *
  * @see
...
object BigNat:
  ...
  /** Safely constructs a BigNat from a BigInt.
    ...
    * @return
    *   `Right(BigNat)` if n ≥ 0, `Left(error message)` if n < 0
    *
    * @example
    *   ```scala
  ...
  /** Converts BigNat to BigInt.
    ...
    *   the BigNat value
    * @return
    *   the underlying BigInt value (always ≥ 0)
    */
  ...
  /** Constructs a BigNat from a BigInt, throwing on negative values.
    ...
    *
    * @note
    *   Use [[fromBigInt]] for safe construction. This method is intended for
    *   cases where the value is known to be valid (e.g., constants, test
    *   fixtures).
    */
  ...
  /** Adds two natural numbers.
    ...
    *
    * @note
    *   Addition of non-negative numbers is always non-negative, so this
  ...
  /** Multiplies two natural numbers.
    ...
    *
    * @note
    *   Multiplication of non-negative numbers is always non-negative, so this
    *   operation never fails.
    */
  ...
  /** Attempts to subtract y from x.
    ...
    *   subtrahend
    * @return
    *   `Right(x - y)` if x ≥ y, `Left(error message)` if x < y
    *
    * @example
    *   ```scala
    ...
    *
    * @note
    *   This is the only arithmetic operation that can fail, as subtraction may
  ...
  /** JSON decoder for BigNat.
    *
    * Decodes from JSON string or number, validating non-negativity. Behavior is
    * controlled by [[org.sigilaris.core.codec.json.JsonConfig]]. By default,
    * accepts both string and numeric representations.
    *
    * @return
    *   decoder instance
    * @see
    *   [[bignatJsonEncoder]] for encoding
    */
  ...
  /** JSON encoder for BigNat.
    *
    * Encodes as JSON string by default (configurable via
    * [[org.sigilaris.core.codec.json.JsonConfig]]).
    *
    * @return
    *   encoder instance
    * @see
    *   [[bignatJsonDecoder]] for decoding
    */
  ...
  /** Byte decoder for BigNat.
    *
    * Decodes from variable-length byte representation. The encoding format is
    * implementation-defined and may include length prefix.
    *
    * @return
    *   decoder instance
    * @see
    *   [[bignatByteEncoder]] for encoding
    */
  given bignatByteDecoder: ByteDecoder[BigNat] = ByteDecoder.bignatByteDecoder

  /** Byte encoder for BigNat.
    *
    * Encodes to variable-length byte representation. The encoding format is
    * implementation-defined and may include length prefix.
    *
    * @return
    *   encoder instance
    * @see
    *   [[bignatByteDecoder]] for decoding
    */
  ...
  /** OrderedCodec instance for BigNat.
    *
    * BigNat's variable-length encoding preserves ordering: smaller natural
    * numbers encode to lexicographically smaller byte sequences.
    *
    * The encoding format uses length-prefixed bytes which satisfies the
    * ordering law: compare(x, y) ≡ encode(x).compare(encode(y))
    *
    * @return
