...
/** Typeclass for encoding/decoding map keys to/from JSON object field names.
  ...
  *
  * @note
  *   Field naming policies from [[JsonConfig]] are NOT applied to map keys.
  *   Keys are encoded/decoded using only the [[JsonKeyCodec]] instance.
  *
  * @see
  *   [[JsonEncoder.mapEncoder]] and [[JsonDecoder.mapDecoder]]
  */
trait JsonKeyCodec[K]:
  ...
  /** Decodes a JSON object field name to a key.
    ...
    *   the field name
    * @return
    *   either a decode failure or the decoded key
    */
  ...
  /** Narrows this codec with decode-time validation only.
    ...
    *         n > 0,
    *         PositiveKey(n),
    *         DecodeFailure(s"Not positive: $n"),
    *       )
    *
    ...
    *   val json = JsonEncoder[Map[PositiveKey, String]].encode(
    *     Map(PositiveKey(1) -> "ok"),
    *   )
    *   val roundtrip = JsonDecoder[Map[PositiveKey, String]].decode(json)
    *   ```
    *
    * @note
    *   If you need total conversions in both directions, prefer [[imap]]. If
    *   you need encode-time validation (L => Either[DecodeFailure, K]), design
    *   your key type so that `from` is total, and validate on `to`.
    */
  final def narrow[L](
      to: K => Either[DecodeFailure, L],
      from: L => K,
  ): JsonKeyCodec[L] =
    new JsonKeyCodec[L]:
      def encodeKey(key: L): String = self.encodeKey(from(key))
