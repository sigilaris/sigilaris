...
object PrefixFreeValidator:
  ...
  /** Checks if byte vector 'a' is a strict prefix of 'b' (a is shorter and
    * matches).
    */
  ...
  /** Validates that all prefixes are prefix-free.
    *
    * Returns:
    *   - Valid if all prefixes are distinct and no prefix is a prefix of
    *     another
  ...
  /** Validates that the given schema (Entry tuples) produces prefix-free table
    * prefixes when mounted at the given path.
    *
    * This is a runtime check that can be used in tests or during module
    * org.sigilaris.core.application.
    */
