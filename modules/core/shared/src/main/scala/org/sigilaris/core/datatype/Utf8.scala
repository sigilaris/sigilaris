...
/** UTF-8 encoded string with length-prefixed byte representation.
  *
  * Utf8 is an opaque type wrapping a String with specialized byte encoding that
  * includes a length prefix ([[BigNat]]) followed by raw UTF-8 bytes. This
  * enables self-describing serialization where the string length is explicitly
  * encoded.
  *
  * Key features:
  ...
  *
  * @note
  *   Byte encoding format: `[size: BigNat][UTF-8 bytes]` where size is the byte
  *   length of the UTF-8 encoded string (not character count).
  * @note
  *   The byte decoder validates UTF-8 encoding and fails with
  *   [[org.sigilaris.core.failure.DecodeFailure]] on invalid sequences.
  * @note
  *   JSON encoding simply delegates to String, providing no additional
  *   structure.
  *
  * @see
  *   [[BigNat]] for the length prefix encoding
  * @see
  *   [[org.sigilaris.core.codec.byte.ByteEncoder]] for byte encoding details
  * @see
  *   [[org.sigilaris.core.codec.json.JsonKeyCodec]] for Map key support
  */
opaque type Utf8 = String

/** Companion object providing constructors and typeclass instances for
  * [[Utf8]].
  */
object Utf8:
  ...
  /** Converts Utf8 to String.
    ...
    */
  extension (u: Utf8) inline def asString: String = u
  ...
  /** Byte encoder for Utf8.
    ...
    * The size represents the byte length of the UTF-8 encoding (not character
    * count).
    *
    * @return
    *   encoder instance
    ...
    *
    * @note
    *   The size prefix uses [[BigNat]] encoding, which is variable-length.
    * @note
    *   Total encoded size is: size of BigNat + UTF-8 byte length
    * @see
    *   [[utf8ByteDecoder]] for decoding
    * @see
    *   [[BigNat]] for size prefix format
    */
    ...
    for
      ...
      res <-
        if remainder.size >= len then
          ...
          DecodeFailure("Insufficient bytes for Utf8 payload")
            .asLeft[DecodeResult[Utf8]]
    yield res
