...
/** UTF-8 encoded string key with order-preserving byte representation.
  *
  * Utf8Key is an opaque type wrapping a String with specialized byte encoding
  * that preserves lexicographic ordering. Unlike [[Utf8]] which uses
  * length-prefix encoding, Utf8Key uses 0x00 terminator with escape sequences
  * to ensure that string comparison order matches byte comparison order.
  *
  * Key features:
  ...
  *
  * @note
  *   Use Utf8Key for table keys and range queries where ordering matters. Use
  *   [[Utf8]] for general value serialization where ordering is not required.
  * @note
  *   The encoding is NOT compatible with Utf8's length-prefix format.
  * @note
  *   Escape overhead: each 0x00 byte in input adds 1 extra byte (0xFF).
  *
  * @see
  *   [[Utf8]] for length-prefixed encoding (does not preserve order)
  * @see
  *   [[org.sigilaris.core.codec.OrderedCodec]] for ordering law details
  */
...
object Utf8Key:
  ...
  /** Converts Utf8Key to String.
    ...
    */
  extension (u: Utf8Key) inline def asString: String = u
  ...
  /** Converts Utf8Key to Utf8.
    ...
    *   the Utf8Key value
    * @return
    *   Utf8 instance with same string value
    */
  extension (u: Utf8Key) def toUtf8: Utf8 = Utf8(u)
  ...
  /** Converts Utf8 to Utf8Key.
    ...
    *   the Utf8 value
    * @return
    *   Utf8Key instance with same string value
    */
  def fromUtf8(u: Utf8): Utf8Key = u.asString

  /** Escapes 0x00 bytes in input by replacing each with 0x00 0xFF.
    *
    * This ensures that the terminator (0x00) can be distinguished from actual
    * 0x00 bytes in the string data.
    *
    * @param bytes
    *   the raw UTF-8 bytes
    * @return
    *   escaped bytes with 0x00 → 0x00 0xFF
    */
  ...
  /** Unescapes 0x00 0xFF sequences back to 0x00 bytes.
    ...
    *   the escaped bytes
    * @return
    *   unescaped bytes with 0x00 0xFF → 0x00
    */
  ...
  private def unescape(bytes: ByteVector): Either[DecodeFailure, ByteVector] =
    ...
    def loop(
        ...
        acc: ByteVector,
    ): Either[DecodeFailure, ByteVector] =
      if remaining.isEmpty then Right[DecodeFailure, ByteVector](acc)
      else if remaining.head == Terminator && remaining.tail.nonEmpty && remaining.tail.head == Escape
      then loop(remaining.drop(2), acc :+ Terminator)
      else if remaining.head == Terminator then
        Left[DecodeFailure, ByteVector](
          DecodeFailure("Invalid escape sequence: 0x00 not followed by 0xFF"),
        )
      else loop(remaining.tail, acc :+ remaining.head)
  ...
  /** Byte encoder for Utf8Key.
    ...
    *   encoder instance
    * @see
    *   [[utf8KeyByteDecoder]] for decoding
    */
  ...
  /** Byte decoder for Utf8Key.
    *
    * Decodes from order-preserving format: escaped UTF-8 bytes + 0x00
    * terminator. Validates UTF-8 encoding and proper escape sequences.
    *
    * @return
    *   decoder instance
    * @note
    *   Fails with [[org.sigilaris.core.failure.DecodeFailure]] if:
    *   - No terminator found
    *   - Invalid escape sequence
    *   - Invalid UTF-8 bytes
    * @see
    *   [[utf8KeyByteEncoder]] for encoding
    */
  @SuppressWarnings(Array("org.wartremover.warts.Nothing"))
  given utf8KeyByteDecoder: ByteDecoder[Utf8Key] = bytes =>
    ...
    def findTerminator(pos: Long): Option[Long] =
      if pos >= bytes.length then None
        ...
        else Some(pos)            // Found unescaped terminator
      else findTerminator(pos + 1)

    findTerminator(0) match
      case None =>
        Left[DecodeFailure, DecodeResult[Utf8Key]](
          DecodeFailure("No terminator found in Utf8Key encoding"),
        )
      case Some(idx) =>
  ...
  /** JSON encoder for Utf8Key.
    ...
    *   encoder instance
    * @see
    *   [[utf8KeyJsonDecoder]] for decoding
    */
  ...
  /** JSON decoder for Utf8Key.
    ...
    *   decoder instance
    * @see
    *   [[utf8KeyJsonEncoder]] for encoding
    */
  ...
  /** JSON key codec for using Utf8Key as Map keys.
    ...
    *   key codec instance
    * @see
    *   [[org.sigilaris.core.codec.json.JsonKeyCodec]] for key codec details
    */
  ...
  /** OrderedCodec instance for Utf8Key.
    *
    * Utf8Key's order-preserving encoding ensures that string lexicographic
    * order matches byte lexicographic order. This satisfies the OrderedCodec
