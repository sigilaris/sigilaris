...
final class TxControlInterpreterSuite extends CatsEffectSuite:
  ...
  test("control batches apply atomically, deduplicate by idempotency key, and expire the retry horizon"):
    for
    ...
    yield
      ...
      assertEquals(sessionAfterFirst.batchingConfig.flushInterval, Duration.ofMillis(5))
      assertEquals(second, Right(ControlBatchOutcome.Deduplicated))
      assertEquals(third, Right(ControlBatchOutcome.Applied))
      assertEquals(stateAfterThird.outboundSessions(sessionId).idempotencyKeys.size, 1)
  ...
  test("rejecting a control batch leaves session state unchanged"):
    ...
    yield
      ...
      assertEquals(after.outboundSessions(sessionId), before.outboundSessions(sessionId))

  test("setFilter replaces prior state and reconnect starts with an empty filter state"):
    val oldFilter = TxBloomFilterSupport.build(Vector.empty, bitsetBytes = 4, numHashes = 2)

    for
      harness <- Harness.create("node-a", startedAt)
      sessionId <- openOutbound(harness)
      firstTx <- harness.source.append(chainId, TestTx("tx-1"), startedAt)
      secondTx <- harness.source.append(chainId, TestTx("tx-2"), startedAt.plusSeconds(1))
      filter1 = TxBloomFilterSupport.build(Vector(firstTx.id), bitsetBytes = 8, numHashes = 2)
      filter2 = TxBloomFilterSupport.build(Vector(secondTx.id), bitsetBytes = 8, numHashes = 2)
      ...
      _ <- harness.runtime.markSessionDead(sessionId).flatMap:
        result =>
  ...
  test("cumulative setKnown entries stay bounded by policy"):
    val known1 = StableArtifactId.unsafeFromBytes(ByteVector.fromByte(0x01.toByte))
    val known2 = StableArtifactId.unsafeFromBytes(ByteVector.fromByte(0x02.toByte))
    val known3 = StableArtifactId.unsafeFromBytes(ByteVector.fromByte(0x03.toByte))
    ...
    yield
      ...
      assertEquals(state.outboundSessions(sessionId).exactKnownIds.getOrElse(chainId, Set.empty), Set(known1))
  ...
  test("setKnown counts distinct ids rather than duplicate input entries"):
    ...
    yield
      ...
      assertEquals(
        state.outboundSessions(sessionId).exactKnownIds.getOrElse(chainId, Set.empty),
        Set(known1, known2),
      )

  test("unresolved bloom positives fall back to explicit backfillUnavailable rejection"):
    for
      harness <- Harness.create("node-a", startedAt)
      sessionId <- openOutbound(harness)
      event <- harness.source.append(chainId, TestTx("tx-1"), startedAt.minusSeconds(5))
      filter = TxBloomFilterSupport.build(Vector(event.id), bitsetBytes = 8, numHashes = 2)
      _ <- harness.runtime.receiveControlBatch(
        sessionId,
        controlBatch(
      ...
      polled <- harness.runtime.pollEvents(sessionId)
    yield
      assertEquals(polled.left.map(_.rejectionClass), Left("backfillUnavailable"))

  test("pollEvents does not overwrite control-state updates committed while polling is in flight"):
    for
  ...
  test("pollEvents returns unknownSession when the session disappears before commit"):
    for
      topology <- IO.fromEither(
        StaticPeerTopology
          .parse(
      ...
      pollFiber <- runtime.pollEvents(sessionId).start
      _ <- started.get
      _ <- runtime.closeSession(sessionId).flatMap(result => IO.fromEither(result.leftMap(r => new IllegalStateException(r.reason))))
      _ <- release.complete(())
      pollResult <- pollFiber.joinWithNever
    yield
      assertEquals(pollResult.left.map(_.rejectionClass), Left("handshakeRejected"))
  ...
  test("event-path session lookup failures use handshake rejection class"):
    ...
    yield
      assertEquals(keepAlive.left.map(_.rejectionClass), Left("handshakeRejected"))
  ...
  test("control keepalive uses control-batch rejection class for unknown sessions"):
    for
      harness <- Harness.create("node-a", startedAt)
      keepAlive <- harness.runtime.controlKeepAlive(DirectionalSessionId.random())
    yield
  ...
  private def openOutbound(
    ...
    for
      ...
      ack <- IO.fromEither(
        SessionNegotiation.acknowledge(
          proposal = proposal,
  ...
  private final case class SequencedBlockingSource(
    ...
    override def readAfter(
      ...
      callCount.modify(count => (count + 1, count)).flatMap:
        ...
        case _ =>
          secondStarted.complete(()).attempt *> secondRelease.get *> delegate.readAfter(chainId, topic, cursor)
