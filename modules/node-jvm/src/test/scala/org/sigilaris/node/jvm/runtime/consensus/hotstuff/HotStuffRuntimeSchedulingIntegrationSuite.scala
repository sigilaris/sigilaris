...
final class HotStuffRuntimeSchedulingIntegrationSuite extends CatsEffectSuite:
  ...
  test("emitProposalFromCandidates stores and emits only schedulable conflict-free block-body selections"):
    ...
    for
      ...
      emission <- IO.fromEither(
        emissionEither.leftMap(rejection => new IllegalStateException(rejection.reason))
      )
      storedView <- blockStore.getView(BlockHeader.computeId(emission.view.header)).value
    yield
      assertEquals(emission.selection.accepted.map(_.tx.id.asString), Vector("a", "b"))
      assertEquals(emission.selection.rejected.map(_.record.tx.id.asString), Vector("compat", "conflict"))
  ...
  test("emitVoteForProposalView rejects proposals whose stored bodies violate scheduling rules"):
    ...
    for
      runtime <- createRuntime()
      blockStore <- BlockStore.inMemory[IO, TestTxRef, Utf8, Utf8]
      view = blockView(body, rootHex = "91", timestampMillis = startedAt.toEpochMilli)
  ...
  test("validated in-memory sink rejects body-visible proposals before acceptance"):
    given GossipClock[IO] = GossipClock.constant[IO](startedAt)
    ...
    for
      ...
      view = blockView(body, rootHex = "92", timestampMillis = startedAt.toEpochMilli)
      _ <- unwrapBlock(blockStore.putView(view))
  ...
  private def unwrapBlock[A](
      result: cats.data.EitherT[IO, org.sigilaris.node.jvm.runtime.block.BlockValidationFailure, A],
  ): IO[A] =
    result.value.flatMap:
      case Left(failure)  => IO.raiseError(new IllegalStateException(failure.reason))
