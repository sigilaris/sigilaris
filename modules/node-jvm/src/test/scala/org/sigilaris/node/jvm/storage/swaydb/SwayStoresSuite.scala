...
final class SwayStoresSuite extends CatsEffectSuite:
  ...
  private def tempDirResource: Resource[IO, Path] =
    Resource.make(
      IO.blocking(Files.createTempDirectory("sigilaris-node-jvm-sway")),
    ) { dir =>
      IO.blocking:
      ...
      SwayStores
        ...
        .use: index =>
          for
            ...
            _ <- IO(
              assertEquals(page, Right(List(Utf8("b") -> 2L, Utf8("c") -> 3L))),
            )
  ...
  test("StorageLayout derives the opinionated SwayDB directory structure"):
    IO:
      ...
      assertEquals(
        layout.block.bestHeader,
        root.resolve("block").resolve("best"),
      )
