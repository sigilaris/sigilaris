...
final class TxGossipArmeriaAdapterSuite extends CatsEffectSuite:
  ...
  test(
    "session-open endpoint accepts a direct neighbor and returns a handshake ack",
  ):
    Harness
      .resource(baseInstant)
      .use: harness =>
        for
          response <- harness.postJson(
            ...
            SessionOpenProposalWire(
              ...
              acceptor = "node-a",
              subscriptions =
                Vector(ChainTopicWire(chainId.value, GossipTopic.tx.value)),
          ...
          ack <- IO.fromEither(
            decode[SessionOpenAckWire](response.body)
  ...
  test(
    "session-open endpoint rejects non-neighbor peers with handshakeRejected",
  ):
    Harness
      .resource(baseInstant)
      .use: harness =>
        for
          ...
          rejection <- IO.fromEither(
            decode[RejectionWire](response.body)
              .leftMap(new IllegalStateException(_)),
          )
        yield
  ...
  test(
    "session-open endpoint requires an authenticated peer header bound to the initiator",
  ):
    Harness
      .resource(baseInstant)
      .use: harness =>
        ...
        for
          ...
          missingRejection <- IO.fromEither(
            decode[RejectionWire](missingResponse.body)
              .leftMap(new IllegalStateException(_)),
          )
          mismatchResponse <- harness.postJson(
            "/gossip/session/open",
            proposal
              .replace("\"initiator\":\"node-b\"", "\"initiator\":\"node-c\""),
            authenticatedPeer = Some("node-b"),
          )
          mismatchRejection <- IO.fromEither(
            decode[RejectionWire](mismatchResponse.body)
              .leftMap(new IllegalStateException(_)),
          )
        yield
  ...
  test(
    "event/control endpoints serve keepalive, requestById, and wrong-channel rejection",
  ):
    Harness
      .resource(baseInstant)
      .use: harness =>
        for
          ...
          keepAliveLines <- decodeBinaryEvents[TestTx](
            keepAliveResponse.bodyBytes,
          )
          controlAckResponse <- harness.postJson(
            s"/gossip/control/${sessionId.value}",
            ControlRequestWire("controlKeepAlive").asJson.noSpaces,
          )
          controlAck <- IO.fromEither(
            decode[ControlResponseWire](controlAckResponse.body)
              .leftMap(new IllegalStateException(_)),
          )
          tx1 <- harness.source.append(
            chainId,
            TestTx("tx-1"),
            baseInstant.minusSeconds(2),
          )
          tx2 <- harness.source.append(
            chainId,
            TestTx("tx-2"),
          ...
          controlBatchAck <- IO.fromEither(
            decode[ControlResponseWire](controlBatchResponse.body)
              .leftMap(new IllegalStateException(_)),
          )
          ...
          requestedLines <- decodeBinaryEvents[TestTx](
            requestedResponse.bodyBytes,
          )
          wrongControlResponse <- harness.postJson(
            s"/gossip/control/${sessionId.value}",
            ControlRequestWire("eventKeepAlive").asJson.noSpaces,
          )
          wrongControlRejection <- IO.fromEither(
            decode[RejectionWire](wrongControlResponse.body)
              .leftMap(new IllegalStateException(_)),
          )
          ...
          wrongEventLines <- decodeBinaryEvents[TestTx](
            wrongEventResponse.bodyBytes,
          )
        yield
          assertEquals(keepAliveResponse.status, 200)
          assert(
            keepAliveResponse.contentType
              .exists(_.startsWith(BinaryEventStreamCodec.MediaType)),
          )
          ...
          assertEquals(
            requestedLines.flatMap(_.event).map(_.payload.body),
            Vector("tx-1"),
          )
          ...
          assertEquals(
            wrongEventLines.flatMap(_.rejection).map(_.reason),
            Vector("wrongChannelMessageKind"),
          )

  test(
    "invalid sessionId path params project handshake rejection on each endpoint family",
  ):
    Harness
      .resource(baseInstant)
      .use: harness =>
        for
          ...
          controlRejection <- IO.fromEither(
            decode[RejectionWire](controlResponse.body)
              .leftMap(new IllegalStateException(_)),
          )
          disconnectResponse <- harness.postNoBody(
            "/gossip/session/not-a-session-id/disconnect",
          )
          disconnectRejection <- IO.fromEither(
            decode[RejectionWire](disconnectResponse.body)
              .leftMap(new IllegalStateException(_)),
          )
        yield
          ...
          assertEquals(
            eventLines.flatMap(_.rejection).map(_.rejectionClass),
            Vector("handshakeRejected"),
          )
          assertEquals(
            eventLines.flatMap(_.rejection).map(_.reason),
            Vector("invalidSessionId"),
          )
  ...
  test(
    "event, control, and disconnect endpoints require the authenticated session peer",
  ):
    Harness
      .resource(baseInstant)
      .use: harness =>
        ...
        yield
          ...
          assertEquals(
            eventLines.flatMap(_.rejection).map(_.reason),
            Vector("missingAuthenticatedPeer"),
          )
          assertEquals(controlResponse.status, 400)
  ...
  test(
    "event endpoint returns HTTP 200 with an in-stream rejection for malformed request bodies",
  ):
    Harness
      .resource(baseInstant)
      .use: harness =>
        ...
        yield
          ...
          assertEquals(
            lines.flatMap(_.rejection).map(_.rejectionClass),
            Vector("handshakeRejected"),
          )
          assertEquals(
            lines.flatMap(_.rejection).map(_.reason),
            Vector("invalidEventRequest"),
          )
  ...
  test("binary event stream codec treats an empty poll as an empty body"):
    val encodedEither =
      BinaryEventStreamCodec.encode(Vector.empty[EventEnvelopeWire[TestTx]])
  ...
  test(
    "binary event stream codec rejects truncated, oversize, unknown-version, and malformed payload frames",
  ):
    ...
    (
      for
        ...
        oversizeFrame =
          ByteEncoder[BigNat]
            .encode(
              BigNat.unsafeFromLong(
                BinaryEventStreamCodec.MaxFrameSizeBytes + 1L,
              ),
            )
  ...
  test(
    "control endpoint returns a control-batch rejection for malformed request bodies",
  ):
    Harness
      .resource(baseInstant)
      .use: harness =>
  ...
  test(
    "control endpoint accepts setFilter base64url and returns deduplicated on repeated idempotency key",
  ):
    Harness
      .resource(baseInstant)
      .use: harness =>
        for
                ...
                ControlBatchWire(
                  ...
                  ops = Vector(
                    ControlOpWire(
                      ...
                      filter = Some(
                        TxBloomFilterWire(
                          bitsetBase64Url = Base64.getUrlEncoder
                            .withoutPadding()
                            .encodeToString(Array(0xfb.toByte)),
                          numHashes = 1,
                          hashFamilyId =
          ...
          body1 <- IO.fromEither(
            decode[ControlResponseWire](response1.body)
              .leftMap(new IllegalStateException(_)),
          )
          ...
          body2 <- IO.fromEither(
            decode[ControlResponseWire](response2.body)
              .leftMap(new IllegalStateException(_)),
          )
        yield
    ...
    Harness
      ...
      .use: harness =>
        for
          ...
          body <- IO.fromEither(
            decode[ControlResponseWire](response.body)
              .leftMap(new IllegalStateException(_)),
          )
        ...
        yield
          ...
          assertEquals(
            replayed.flatMap(_.event).map(_.payload.body),
            Vector("tx-2"),
          )
  ...
  test(
    "half-open recovery reuses the existing peer correlation id and keeps the surviving direction live",
  ):
    MeshHarness
      .resource(baseInstant)
      .use: mesh =>
        for
          ...
          _ <- mesh.a.postNoBody(
            s"/gossip/session/${aToB.proposal.sessionId.value}/disconnect",
          )
          _ <- mesh.b.postNoBody(
            s"/gossip/session/${aToB.proposal.sessionId.value}/disconnect",
          )
          relationshipHalfOpenA <- mesh.a.runtime.relationshipWith(
            PeerIdentity.unsafe(mesh.b.localNodeId),
          )
          relationshipHalfOpenB <- mesh.b.runtime.relationshipWith(
            PeerIdentity.unsafe(mesh.a.localNodeId),
          )
          recoveryProposalEither <- mesh.a.runtime.startOutbound(
            PeerIdentity.unsafe(mesh.b.localNodeId),
            subscription,
          )
          recoveryProposal <- IO.fromEither(
            recoveryProposalEither
              .leftMap(rejection => new IllegalStateException(rejection.reason)),
          )
          _ <- mesh.b.source.append(
            chainId,
            TestTx("surviving"),
            baseInstant.minusSeconds(1),
          )
          ...
          survivingAck <- IO.fromEither(
            decode[ControlResponseWire](survivingControl.body)
              .leftMap(new IllegalStateException(_)),
          )
          ...
          recoveryAckWire <- IO.fromEither(
            decode[SessionOpenAckWire](recoveryResponse.body)
              .leftMap(new IllegalStateException(_)),
          )
          recoveryAck <- IO.fromEither(
            toAck(recoveryAckWire).leftMap(new IllegalArgumentException(_)),
          )
          ...
          relationshipOpenA <- mesh.a.runtime.relationshipWith(
            PeerIdentity.unsafe(mesh.b.localNodeId),
          )
          relationshipOpenB <- mesh.b.runtime.relationshipWith(
            PeerIdentity.unsafe(mesh.a.localNodeId),
          )
        yield
          assertEquals(
            relationshipHalfOpenA.map(_.status),
            Some(PeerRelationshipStatus.HalfOpen),
          )
          assertEquals(
            relationshipHalfOpenB.map(_.status),
            Some(PeerRelationshipStatus.HalfOpen),
          )
          assertEquals(
            recoveryProposal.peerCorrelationId,
            aToB.proposal.peerCorrelationId,
          )
          assert(recoveryProposal.sessionId != aToB.proposal.sessionId)
          assertEquals(survivingPoll.status, 200)
          assertEquals(
            survivingLines.flatMap(_.event).map(_.payload.body),
            Vector("surviving"),
          )
          assertEquals(survivingControl.status, 200)
          assertEquals(survivingAck.status, "ack")
          assertEquals(
            relationshipOpenA.map(_.status),
            Some(PeerRelationshipStatus.Open),
          )
          assertEquals(
            relationshipOpenB.map(_.status),
            Some(PeerRelationshipStatus.Open),
          )

  test(
    "reconnect starts with an empty filter state and does not carry over prior setFilter state",
  ):
    Harness
      .resource(baseInstant)
      .use: harness =>
        for
                    ...
                    ControlOpWire(
                      ...
                      filter = Some(
                        TxBloomFilterWire(
                          bitsetBase64Url = Base64.getUrlEncoder
                            .withoutPadding()
                            .encodeToString(filter.bitset.toArray),
          ...
          _ <- IO.fromEither(
            decode[ControlResponseWire](filterResponse.body)
              .leftMap(new IllegalStateException(_)),
          )
          filteredState <- harness.runtime.snapshotState
          _ <- harness.postNoBody(
            s"/gossip/session/${session1.value}/disconnect",
          )
        ...
        yield
          ...
          assertEquals(
            pollLines.flatMap(_.event).map(_.payload.body),
            Vector("known"),
          )
          assertEquals(
            filteredState.outboundSessions(session1).filters.get(chainId),
            Some(filter),
          )
          assertEquals(state.engine.sessionById(session1), None)
          assertEquals(state.outboundSessions.get(session1), None)
          assertEquals(
            state.outboundSessions(session2).filters.get(chainId),
            None,
          )

  test(
    "opening timeout and open-session idle timeout move sessions to dead without manual disconnect",
  ):
    (
      for
      ...
      yield
        ...
        assertEquals(
          idleLines.flatMap(_.rejection).map(_.reason),
          Vector("sessionNotOpen"),
        )

  test(
    "pre-open event and control traffic reject and close the opening lineage over HTTP",
  ):
    Harness
      .resource(baseInstant)
      .use: harness =>
        ...
        yield
          ...
          assertEquals(
            eventLines.flatMap(_.rejection).map(_.reason),
            Vector("preOpenEventTraffic"),
          )
          assertEquals(
            stateAfterEvent.engine
              .sessionById(openingEventSession)
              .map(_.status),
          ...
          assertEquals(
            stateAfterControl.engine
              .sessionById(openingControlSession)
              .map(_.status),
            Some(DirectionalSessionStatus.Closed),
          )
  ...
  private def decodeBinaryFailure[A: ByteDecoder](
    ...
    BinaryEventStreamCodec.decode[A](body) match
      ...
      case Right(value) =>
        IO.raiseError(new IllegalStateException(value.toString))
  ...
  private final case class Harness(
    ...
    def openOutboundLocally(
      ...
      for
        proposalEither <- runtime.startOutbound(
          PeerIdentity.unsafe(remoteNodeId),
          subscription,
        )
        proposal <- IO.fromEither(
          proposalEither.leftMap(rejection =>
  ...
  private given ByteDecoder[TestTx] =
    ByteDecoder[Utf8].map(value => TestTx(value.asString))
