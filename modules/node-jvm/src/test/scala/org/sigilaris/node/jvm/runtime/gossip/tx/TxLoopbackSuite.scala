...
final class TxLoopbackSuite extends CatsEffectSuite:
  ...
  test("loopback handshake delivers events, resumes from setCursor, and preserves timestamps"):
    for
      ...
      _ <- a.runtime.receiveControlBatch(
        ...
        controlBatch(
          ...
          Vector(ControlOp.SetCursor(CompositeCursor(Map(ChainTopic(chainId, GossipTopic.tx) -> lastCursor)))),
        ),
      )
    ...
    yield
      ...
      assertEquals(received1.toOption.get.applied.map(_.payload.body), Vector("tx-1", "tx-2"))
      assertEquals(events2.map(_.payload.body), Vector("tx-3"))
      assertEquals(received2.toOption.get.applied.map(_.payload.body), Vector("tx-3"))

  test("loopback surfaces stale cursors and deduplicates control retries within the horizon"):
    for
      a <- Harness.create("node-a", "node-b", baseInstant)
      b <- Harness.create("node-b", "node-a", baseInstant)
      opened <- openOutbound(a, b)
      ...
      batch = controlBatch(
        ...
        Vector(ControlOp.SetCursor(CompositeCursor(Map(ChainTopic(chainId, GossipTopic.tx) -> invalidCursor)))),
      )
  ...
  test("loopback honors setKnown, explicit requestById fetches, and sink dedupes newer cursors with the same stable id"):
    for
      a <- Harness.create("node-a", "node-b", baseInstant)
      b <- Harness.create("node-b", "node-a", baseInstant)
      opened <- openOutbound(a, b)
      sessionId = opened.proposal.sessionId
      txOther <- a.source.append(chainId, TestTx("other"), baseInstant.minusSeconds(7))
      ...
      _ <- a.runtime.receiveControlBatch(
        ...
        controlBatch(
          ...
          Vector(
            ControlOp.SetCursor(CompositeCursor(Map(ChainTopic(chainId, GossipTopic.tx) -> lastCursor))),
      ...
      received2 <- b.runtime.receiveEvents(sessionId, messages2)
    yield
      assertEquals(extractEvents(messages1).map(_.payload.body), Vector("same", "same"))
      assertEquals(received1.toOption.get.applied.map(_.payload.body), Vector("same"))
      assertEquals(received1.toOption.get.duplicates.map(_.payload.body), Vector("same"))
      assertEquals(extractEvents(messages2).map(_.payload.body), Vector("other"))
      assertEquals(received2.toOption.get.applied.map(_.payload.body), Vector("other"))

  test("reverse direction reuses the peer correlation id and half-open relationship is preserved after one side dies"):
    for
      a <- Harness.create("node-a", "node-b", baseInstant)
      b <- Harness.create("node-b", "node-a", baseInstant)
      openedAtoB <- openOutbound(a, b)
      proposalBtoAEither <- b.runtime.startOutbound(PeerIdentity.unsafe("node-a"), subscription)
      proposalBtoA <- IO.fromEither(proposalBtoAEither.leftMap(rejection => new IllegalStateException(rejection.reason)))
      ...
      _ <- b.runtime.applyHandshakeAck(acceptedOnA.ack).flatMap:
        result =>
          IO.fromEither(result.leftMap(rejection => new IllegalStateException(rejection.reason)))
      relationshipOpen <- a.runtime.relationshipWith(PeerIdentity.unsafe("node-b"))
      _ <- a.runtime.markSessionDead(openedAtoB.proposal.sessionId).flatMap:
        result =>
          IO.fromEither(result.leftMap(rejection => new IllegalStateException(rejection.reason)))
      relationshipHalfOpen <- a.runtime.relationshipWith(PeerIdentity.unsafe("node-b"))
    yield
      assertEquals(proposalBtoA.peerCorrelationId, openedAtoB.proposal.peerCorrelationId)
      assertEquals(relationshipOpen.map(_.status), Some(PeerRelationshipStatus.Open))
      assertEquals(relationshipHalfOpen.map(_.status), Some(PeerRelationshipStatus.HalfOpen))
  ...
  test("config batching applies the maxBatchItems cap and flushInterval timer"):
    for
      ...
      _ <- a.runtime.receiveControlBatch(
        ...
        controlBatch(
          ...
          Vector(
            ControlOp.SetCursor(CompositeCursor(Map(ChainTopic(chainId, GossipTopic.tx) -> secondCursor))),
            ControlOp.Config(
              Map(
    ...
    yield
      ...
      assertEquals(extractEvents(delayedPoll.toOption.get).map(_.payload.body), Vector("tx-3", "tx-4"))

  test("requestById emission is capped by maxBatchItems and drains across polls"):
    for
      a <- Harness.create("node-a", "node-b", baseInstant)
      b <- Harness.create("node-b", "node-a", baseInstant)
      opened <- openOutbound(a, b)
      sessionId = opened.proposal.sessionId
      tx1 <- a.source.append(chainId, TestTx("tx-1"), baseInstant.minusSeconds(3))
      tx2 <- a.source.append(chainId, TestTx("tx-2"), baseInstant.minusSeconds(2))
      tx3 <- a.source.append(chainId, TestTx("tx-3"), baseInstant.minusSeconds(1))
      _ <- a.runtime.receiveControlBatch(
        sessionId,
        controlBatch(
  ...
  test("explicit requestById delivery does not move the stream cursor backwards"):
    for
      ...
      _ <- a.runtime.receiveControlBatch(
        ...
        controlBatch(
          ...
          Vector(
            ControlOp.SetCursor(CompositeCursor(Map(ChainTopic(chainId, GossipTopic.tx) -> latestCursor))),
    ...
    yield
      assertEquals(extractEvents(requestMessages).map(_.payload.body), Vector("tx-1"))
      assertEquals(nextPoll, Right(Vector.empty))

  test("nack replays from the requested cursor without advancing the durable checkpoint"):
    for
      a <- Harness.create("node-a", "node-b", baseInstant)
      b <- Harness.create("node-b", "node-a", baseInstant)
      opened <- openOutbound(a, b)
      ...
      nextPoll <- a.runtime.pollEvents(sessionId)
    yield
      assertEquals(extractEvents(replayMessages).map(_.payload.body), Vector("tx-2", "tx-3"))
      assertEquals(replayReceive.toOption.get.duplicates.map(_.payload.body), Vector("tx-2", "tx-3"))
      assertEquals(nextPoll, Right(Vector.empty))

  test("nack with no cursor replays the full chain from the origin immediately"):
    for
      a <- Harness.create("node-a", "node-b", baseInstant)
      b <- Harness.create("node-b", "node-a", baseInstant)
      opened <- openOutbound(a, b)
