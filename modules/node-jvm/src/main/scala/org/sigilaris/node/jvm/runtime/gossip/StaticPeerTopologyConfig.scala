...
object StaticPeerTopologyConfig:
  ...
  def loadSection(
    ...
    for
      localNodeIdentity <- requiredString(section, "local-node-identity", "localNodeIdentity")
      knownPeers <- requiredStringList(section, "known-peers", "knownPeers")
      directNeighbors <- requiredStringList(section, "direct-neighbors", "directNeighbors")
