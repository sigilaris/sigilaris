...
/** Factory for creating SwayDB-backed persistent store resources with automatic
...
object SwayStores:
  ...
  /** Creates a SwayDB-backed `KeyValueStore` resource that is closed when the
    ...
    * @return
    *   a resource that yields a persistent key-value store
    */
  ...
  /** Creates a SwayDB-backed `SingleValueStore` resource.
    ...
    *   the filesystem directory for the SwayDB storage
    * @return
    *   a resource that yields a persistent single-value store
    */
  ...
  /** Creates a SwayDB-backed `StoreIndex` resource with range query support.
    ...
    *   the filesystem directory for the SwayDB storage
    * @return
    *   a resource that yields a persistent store index
    */
