...
object InMemoryStores:
  ...
  /** Creates an in-memory `KeyValueStore` resource backed by a `Map`.
    *
    * @tparam F
    *   the effect type with `Sync` capability
    * @tparam K
    *   the key type
    * @tparam V
    *   the value type
    * @return
    *   a resource that yields a new in-memory key-value store
    */
  ...
  /** Creates an in-memory `StoreIndex` resource backed by a `TreeMap` for
    * ordered access.
    *
    * @tparam F
    *   the effect type with `Sync` capability
    ...
    *   the value type
    * @return
    *   a resource that yields a new in-memory store index
    */
  ...
  /** Creates an in-memory `SingleValueStore` resource.
    ...
    *   the value type
    * @return
    *   a resource that yields a new in-memory single-value store
    */
