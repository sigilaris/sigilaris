...
final case class VoteAccumulator(
    votesById: Map[VoteId, Vote] = Map.empty,
    votesByEquivocationKey: Map[EquivocationKey, Vote] = Map.empty,
):
  def record(
      vote: Vote,
    ...
    votesById.get(vote.voteId) match
      case Some(_) =>
        Right(this -> VoteRecordOutcome.Duplicate)
      case None =>
          ...
          case Some(existing) if existing.targetProposalId != vote.targetProposalId =>
            Left(
              HotStuffValidationFailure(
                ...
                detail = Some(
                  s"${vote.equivocationKey.validatorId.value}:${existing.targetProposalId.toHexLower}:${vote.targetProposalId.toHexLower}"
                ),
              )
            )
          case Some(_) =>
          ...
          case None =>
            Right(
              copy(
                ...
                votesByEquivocationKey = votesByEquivocationKey.updated(vote.equivocationKey, vote),
...
object VoteAccumulator:
  val empty: VoteAccumulator = VoteAccumulator()
  ...
  def assemble(
    ...
    val deduplicated =
      votes
        ...
        ._2
    for
      byValidator <- deduplicated.foldLeft[Either[HotStuffValidationFailure, Map[ValidatorId, Vote]]](Right(Map.empty)):
        case (Right(acc), vote) =>
      ...
      _ <- HotStuffValidationSupport.ensure(
        byValidator.size >= validatorSet.quorumSize,
        "insufficientQuorum",
        Some(s"required=${validatorSet.quorumSize} actual=${byValidator.size}"),
      )
...
object HotStuffValidator:
  def validateProposal(
    ...
    for
      ...
      proposer <- validatorSet.member(proposal.proposer).toRight(
        HotStuffValidationFailure(
  ...
  def validateVote(
    ...
    for
      ...
      voter <- validatorSet.member(vote.voter).toRight(
        HotStuffValidationFailure(
          reason = "unknownVoter",
          detail = Some(vote.voter.value),
        )
  ...
  def validateQuorumCertificate(
    ...
    for
      ...
      _ <- QuorumCertificateAssembler.assemble(qc.subject, qc.votes, validatorSet).void
    yield ()
