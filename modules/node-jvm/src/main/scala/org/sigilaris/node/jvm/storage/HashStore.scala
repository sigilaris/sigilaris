...
/** Content-addressable store that uses cryptographic hashes as keys.
  ...
  *   the effect type
  * @tparam A
  *   the type of values stored, which must be hashable
  */
trait HashStore[F[_], A]:
  ...
  /** Retrieves a value by its hash.
    *
    * @param hash
    *   the hash of the value to look up
    * @return
  ...
  /** Removes the value associated with the given hash.
    *
    * @param hash
    *   the hash of the value to remove
    */
...
object HashStore:
  ...
  /** Derives a `HashStore` from an existing `KeyValueStore` keyed by hash
    * values.
    *
    * @tparam F
    *   the effect type
    * @tparam A
    *   the value type, which must have a `Hash` instance
    * @param kvStore
    *   the underlying key-value store
    * @return
    *   a `HashStore` backed by the given key-value store
    */
