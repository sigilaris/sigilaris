...
- 진행 상황: `SignedTxBuilder` 도입 완료, `SignedTxBuilderTest`를 통해 계정 모듈 경로에서 정상 동작 검증.
...
- 진행 상황: 계정 모듈에 요구되는 증거는 수동으로 정비 완료. Groups 리듀서 마이그레이션 이후 본 헬퍼 적용 예정.
...
- 진행 상황: Accounts 트랜잭션/테스트에 브랜드 래퍼 적용 완료(`AccountsBranding.scala`). Groups 모듈은 마이그레이션 대기.
...
- 진행 상황: `SignatureVerifier` 통합 완료, Accounts/Groups 리듀서에서 공통 헬퍼 사용하도록 정비 완료. 차기 마일스톤에서는 토큰/기타 모듈 적용 범위를 확대한다. (완료)
- `StateT` 에러 리프팅 헬퍼: `StateT.liftF(EitherT.leftT(...))` 패턴을 반복적으로 사용하므로 `StoreF.raise(failure)` 헬퍼를 추가해 가독성과 실수를 줄인다.
- 진행 상황: `StoreF.pure`/`lift`/`raise` 헬퍼 보강 완료, 모나드 제약 및 Accounts 경로 적용 확인.
