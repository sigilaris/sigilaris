...
1. **Sigilaris canonical block family는 `BlockHeader`, `BlockBody`, `BlockView` 계층으로 분리한다.**
   ...
   - application-facing ordered body는 `BlockBody`다.
...
2. **`BlockHeader` baseline은 최소한 아래 semantic field를 포함해야 한다.**
   ...
   - `bodyRoot`는 canonical ordered block body commitment이며, auxiliary receipt/event sub-root는 baseline mandatory field로 두지 않는다.
...
9. **body verification은 `bodyRoot`와 `BlockBody` 사이의 deterministic commitment contract로 수행한다.**
   - body ordering은 canonical해야 하며 implementation-local set semantics에 의존해서는 안 된다.
   - `BlockView` receiver는 필요 시 `bodyRoot == hash(canonical-encoded body)` 또는 동등한 검증을 수행할 수 있어야 한다.
